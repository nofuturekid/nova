...
F1–F4 implemented together as the combined beta **0.1.32-beta1** (one
device-gated beta, mirroring the Amendment's combined-acceptance model);
device-acceptance pending (ADR-0027 Tier 3). Status stays Accepted.

- **F1 — `SettingsScreen` scroll:** plain `Column` (no `verticalScroll`), clips on short screens (≈360×640dp). Add `verticalScroll`/lazy. (triage #13) — **Implemented 0.1.32-beta1**: `verticalScroll(rememberScrollState())` added to the inner content `Column` (the cards container has no `weight()`/`fillMaxHeight`/nested scrollable child, so no infinite-height risk; header `Row` stays fixed). Zero-visual on tall screens, scrolls on short ones. Maintainer device-accepted 2026-05-18 (combined beta v0.1.32-beta1, provisional; stable promotion still maintainer-only).
- **F2 — on-accent literal `Color(0xFF06120E)`** at ArrayTab/ContainerDetailSheet(×2)/VmDetailSheet/VmsTab(×2): bypasses `onPrimary`/`onTone()` → invisible on light accents (Rule 13). Route through P1 helpers. (triage #15) — **Implemented 0.1.32-beta1**: all 6 sites now use the P1/P3 single-source-of-truth `onTone(tone)` (`ComponentColors.kt`): `onTone(t.accent)` for the accent-toned buttons/gradients, `onTone(t.info)` for the `Tone.Info` update button. INTENDED visible fix on light accents (e.g. Amber); ~identical on dark. `SnapshotStates.kt:108` left untouched (outside triage #15). Maintainer device-accepted 2026-05-18 (combined beta v0.1.32-beta1, provisional; stable promotion still maintainer-only).
- **F3 — bespoke `Toggle`** in `SettingsScreen` (hardcoded white thumb, no `Role.Switch`/`toggleable`) → M3 `Switch`. (triage #16) — **Implemented 0.1.32-beta1**: `Toggle` body swapped to M3 `Switch` (signature/call site unchanged); colours via new P1-style `unraidSwitchColors()` (`ComponentColors.kt`) derived from the live theme (`accent` track, `onTone(accent)` checked thumb, `text @ controlTrackOff` off-track — no literals). Intended M3 look change (Rule 13) + free `Role.Switch` a11y. Maintainer device-accepted 2026-05-18 (combined beta v0.1.32-beta1, provisional; stable promotion still maintainer-only).
- **F4 — `TabRow` deprecated → `PrimaryTabRow`/`SecondaryTabRow`:** `NotificationsSheet` (~:122) uses the deprecated M3 `TabRow`. Swap to the M3 replacement when the roadmap reaches it. (compiler-deprecation surfaced 2026-05-18) — **Implemented 0.1.32-beta1**: swapped to `SecondaryTabRow` (correct M3 idiom for in-sheet content-filter tabs; Primary* is for top-level destinations); `containerColor`/`contentColor` preserved. One intentional M3 difference: the secondary indicator uses `colorScheme.primary` (the P1-wired accent) instead of the legacy default `contentColor`. Maintainer device-accepted 2026-05-18 (combined beta v0.1.32-beta1, provisional; stable promotion still maintainer-only).
